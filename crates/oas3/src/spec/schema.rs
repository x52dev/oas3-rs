...
///
/// See <https://spec.openapis.org/oas/v3.1.1#schema-object> and
...
pub struct ObjectSchema {
    ...
    ///
    /// This keyword interacts with `prefixItems`, where it can be either:
    /// - A boolean value: `false` means no additional items allowed, `true` means any additional
    ///   items allowed;
    ...
    /// Validation succeeds if each element of the instance validates against the schema at the same
    /// position, if any.
    ///
    /// This keyword does not constrain the length of the array. If the array is longer than this
    ...
    ///
    /// [Schema Object]: https://spec.openapis.org/oas/v3.1.1#schema-object
    #[serde(
    ...
    // https://spec.openapis.org/oas/v3.1.1#fixed-fields-20
    // #########################################################################
    ...
    pub fn resolve(&self, spec: &Spec) -> Result<Self, RefError> {
        match self {
            ...
            Schema::Object(obj_ref) => match obj_ref.as_ref() {
                ...
                ObjectOrReference::Ref { ref_path, .. } => Schema::from_ref(spec, ref_path),
            },
        }
    }
}
    ...
    fn from_ref(spec: &Spec, path: &str) -> Result<Self, RefError> {
        ...
        match refpath.kind {
            RefType::Schema => spec
                ...
                .and_then(|schema| schema.resolve(spec)),
...
mod tests {
    ...
    #[test]
    fn prefix_items_supports_inline_and_reference_schemas() {
        let spec = indoc::indoc! {"
          type: array
          prefixItems:
            - type: string
        ...
        assert_eq!(
            ...
            "prefixItems should only parse to two elements",
        );
    ...
    #[test]
    fn prefix_items_supports_empty_list() {
        let spec = indoc::indoc! {"
          type: array
          prefixItems: []
        "};
    ...
    #[test]
    fn items_supports_inline_schema() {
        let spec = indoc::indoc! {"
          type: array
          items:
            type: number
        ...
        let items = schema
            .items
            .expect("Parsed schema should contain items property");

        let Schema::Object(obj) = *items else {
        ...
        assert_matches!(
            &*obj,
            ObjectOrReference::Object(ObjectSchema {
                schema_type: Some(TypeSet::Single(Type::Number)),
                ..
            }),
            "items should be inline number schema",
        );
    }

    #[test]
    fn items_supports_boolean_schema() {
        let spec = indoc::indoc! {"
          type: array
          items: true
        "};
        ...
        assert_matches!(
            *items,
            ...
            "items should be boolean schema",
        );
    }
    ...
    #[test]
    fn properties_support_boolean_schemas() {
        let spec = indoc::indoc! {"
          type: object
          properties:
            data: true
        ...
        assert_matches!(
            ...
            Some(Schema::Boolean(BooleanSchema(false))),
            "meta property should be boolean schema with value false",
        );
    }

    #[test]
    fn composition_keywords_support_boolean_schemas() {
        let spec = indoc::indoc! {"
          allOf:
            - type: string
            - true
    ...
    #[test]
    fn prefix_items_support_boolean_schemas() {
        let spec = indoc::indoc! {"
          type: array
          prefixItems:
            - true
        ...
        assert_eq!(3, schema.prefix_items.len());
